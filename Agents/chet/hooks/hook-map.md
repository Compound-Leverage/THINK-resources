...
(Claude scheduled routine or ChatGPT Scheduled Task), see
`Workflows/cluster-discovery-chet.md`, which already documents Chet's cadence. Don't
duplicate that setup guidance here; this file just tells you which skill each trigger
should point at.

| When to run | Trigger | Fires |
|---|---|---|
| Weekly, once | Scheduled routine (see `Workflows/cluster-discovery-chet.md`) | `Skills/cluster-discovery/` -- discover new groups for unmapped capabilities, refresh existing known groups (membership, budget confirmation, window estimate), archive anything whose window has closed without conversion |
...
`Workflows/cluster-discovery-chet.md` for the full explanation and the durability note
