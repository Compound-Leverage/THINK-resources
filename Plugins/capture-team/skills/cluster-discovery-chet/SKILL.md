...
Chet finds what might be worth pursuing — named groups and organizations, markets,
government contracting opportunities, grant and funding opportunities, or other bounded
pursuits — based on the criteria you configure. Starting from a known capability your team
already knows how to deliver and hunting for every named group carrying that exact gap is
one discovery method Chet uses, not the entire definition of what he does — broaden
`capability_map` and `scanning.sources` to match however you define "worth pursuing" in
your own context.
