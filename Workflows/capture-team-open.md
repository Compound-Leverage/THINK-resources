# Capture Team (capture-team-open) -- Automation Setup

How to put Chet, Kipp, and Ben on a recurring schedule instead of running them by hand.
...
just runs the same prompts from [`Prompts/capture-team-open.md`](../Prompts/capture-team-open.md)
on a timer.
...
|---|---|---|
| Chet -- Cluster Discovery | Weekly (once) | "Load our capability map and check for new signals matching our unmapped capabilities. Refresh existing tracked groups: update membership, budget confirmation, and window estimates, and archive anything whose window has closed without conversion." |
| Kipp -- CRM Intake | Daily | "Pull the new leads sitting in our intake queue, enrich each one, classify it against our ICP profile, and create or update the record in our CRM. Flag any conflict with a manually-edited field instead of overwriting it." |
| Ben -- Signal Delivery | Daily or on new signal | "Read the new, unactioned signals in our signal inbox and draft a client-ready intelligence brief for any client with a new signal, using our brief section structure. Flag exceptions instead of guessing." |

Name each routine after the persona (e.g. `chet-cluster-discovery`) so `/schedule list`
...
**Optional -- run all three as a pipeline instead of independently:** if you want Ben's
brief to always reflect the freshest clusters and contacts, chain them in one routine
instead of three separate ones: Chet refreshes clusters first, Kipp intakes and classifies
next, Ben drafts briefs last. Each step should validate the previous one's output landed
before continuing, the same way you'd gate any multi-step pipeline.

**Three habits worth keeping once a routine is live:**

- **Short-circuit on empty.** Before Kipp runs the full enrichment/classification steps,
  have it check the intake queue first -- if there's nothing new, post "0 records" and
  exit rather than burning a run on nothing.
- **The routine's stored prompt is a frozen copy, not a live link back to your files.**
  If you edit `customization/my-capture-config.json` or change how you're prompting a
  persona, update the scheduled routine's prompt too -- editing the repo alone won't
...
- **End every run with a summary, not a raw dump.** A short notification (records
  processed, ICP/routing breakdown, anything flagged for manual review) is more useful
  than the full output -- save the full record for wherever Kipp/Ben already write it
  (your CRM, your Drive folder).
...
routine with MCP access can -- Kipp and Ben's outputs will need a manual copy/paste step
