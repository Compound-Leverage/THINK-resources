...
Kipp -- CRM Intake. Suggested cadence: daily. Trigger prompt: "Pull the new leads sitting
in our intake queue, enrich each one, classify it against our ICP profile, and create or
update the record in our CRM. Flag any conflict with a manually-edited field instead of
overwriting it."

Name the routine `crm-intake-kipp` so `/schedule list` stays readable once you're running
...
thin: "Read `Plugins/capture-team/skills/crm-intake-kipp/SKILL.md` and
...
This is often run as part of a pipeline with Chet and Ben -- see
[`Workflows/signal-delivery-ben.md`](signal-delivery-ben.md) for the full sequence.
...
- **The routine's stored prompt is a frozen copy, not a live link back to your files.**
  If you edit `customization/my-capture-config.json` or change how you're prompting Kipp,
...
- **End every run with a summary, not a raw dump.** A short notification (records
  ...
  than the full output -- save the full record for wherever Kipp already writes it (your
  CRM).
...
1. Open a conversation with Kipp's `SKILL.md` uploaded (see
   [`Skills/README.md`](../Skills/README.md) for upload steps)
2. Say: "Schedule this to run daily and use this prompt each time: [trigger prompt
   above]"
...
with MCP access can -- Kipp's output will need a manual copy/paste step unless you've
