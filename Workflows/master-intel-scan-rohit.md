# Rohit (content-team) -- Automation Setup

How to put Rohit on a recurring schedule instead of running him by hand. Two options
...
Name the routine after the persona (e.g. `rohit-intel-scanner`) so `/schedule list`
...
`Plugins/content-team/skills/master-intel-scan-rohit/SKILL.md` and your
...
**Pipeline note -- Rohit runs first.** The 9 roles in this plugin don't all sit at the
same dependency level, so if you're running more than one, don't just chain them in list
...
- **The routine's stored prompt is a frozen copy, not a live link back to your files.**
  ...
  change how you're prompting Rohit, update the scheduled routine's prompt too --
...
- **End every run with a summary, not a raw dump.** A short notification (topics scored,
  ...
  save the full record for wherever the skill already writes it (your content DB, your
...
1. Open a conversation with Rohit's `SKILL.md` uploaded (see
   [`Skills/README.md`](../Skills/README.md) for upload steps)
2. Say: "Schedule this to run weekly and use this prompt each time: [trigger prompt
...
Code routine with MCP access can -- Rohit's output will need a manual copy/paste step
unless you've connected the equivalent tools. If you're running more than one persona
this way, the pipeline gating above (Rohit before the parallel writers, Cal last) also
has to be run manually in that order, since ChatGPT Scheduled Tasks don't gate one
task's start on another task's completion.
