...
ChatGPT's Scheduled Tasks. Pick one - you don't need both.
...
Porter only makes sense triggered on Assessment Package approval, not a fixed clock - set
...
Name the routine `proposal-strategist-porter` so `/schedule list` stays readable once
...
`Plugins/proposal-team/skills/proposal-strategist-porter/SKILL.md` and your
...
- **Short-circuit on empty.** Before Porter runs, have it check whether an Assessment
...
- **The routine's stored prompt is a frozen copy, not a live link back to your files.**
  If you edit `customization/my-case-studies.json` or any other customization file,
...
- **End every run with a summary, not a raw dump.** A short notification (win themes
  picked, case studies matched, elevator pitch) is more useful than the full Strategy
  Package dropped into chat - save the full document for wherever your CRM or Drive
  folder already keeps deal records.
...
1. Open a conversation with Porter's `SKILL.md` uploaded (see
   [`Skills/README.md`](../Skills/README.md) for upload steps)
2. Say: "Schedule this to run when I tell you to and use this prompt each time: Using
   the Discovery Brief and Assessment Package for [prospect name], develop our
   competitive positioning, 3-5 win themes, and pick our top 3 matching case studies."
...
routine with MCP access can - Porter's case study matching will need a manual copy/paste
