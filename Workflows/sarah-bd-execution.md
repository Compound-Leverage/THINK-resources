# Sarah - BD Execution (sales-bd-team-open) -- Automation Setup

How to put Sarah on a recurring schedule instead of running her by hand. Two options
...
Name each routine after the persona (e.g. `sarah-bd-execution`) so `/schedule list` stays
...
`Plugins/sales-bd-team-open/skills/sarah-bd-execution/SKILL.md` and your
`customization/` files in this repo, then run today's outreach pass." Claude Code CCRs
...
`Workflows/lori-prospect-scout.md` for the full sequence.
...
1. Open a conversation with Sarah's `SKILL.md` uploaded (see
...
Claude Code routine with MCP access can - Sarah's outputs will need a manual copy/paste
