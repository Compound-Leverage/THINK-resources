# Ben (capture-team) -- Automation Setup
...
Name the routine `signal-delivery-ben` so `/schedule list` stays readable once you're
...
thin: "Read `Plugins/capture-team/skills/signal-delivery-ben/SKILL.md` and
...
multi-step pipeline. See `Workflows/cluster-discovery-chet.md` and
`Workflows/crm-intake-kipp.md` for those two personas' individual setup.
